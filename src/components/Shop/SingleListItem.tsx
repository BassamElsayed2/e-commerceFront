...
const SingleListItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group rounded-lg bg-[#DFE0E2] shadow-md transition-transform duration-300 hover:scale-105">
      <div className="flex flex-col sm:flex-row">
        <div className="relative flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4 overflow-hidden shadow-sm">
          <Image
            src={
          ...
          />
          <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2 pb-5 transition-transform duration-300 group-hover:translate-y-0">
            <button
              onClick={() => {
                openModal();
              ...
              className="flex items-center justify-center w-9 h-9 rounded-md shadow-md text-[#0C2756] bg-[#DFE0E2] hover:bg-[#239FBF] hover:text-white transition-colors duration-200"
            >
              <svg
            ...
            <button
              ...
              className="px-5 py-2 rounded-md bg-[#239FBF] text-white font-medium hover:bg-[#0C2756] transition-colors duration-200"
            >
        ...
        <div className="flex flex-col gap-3 sm:gap-5 sm:flex-1 sm:pl-6 py-5 px-4">
          <h3 className="font-medium text-[#0C2756] text-lg hover:text-[#239FBF] transition-colors duration-200">
            <Link href={`/shop-details?id=${item.id}`} onClick={handleProductDetailsUpdate}>
              {locale === "ar" ? item.name_ar : item.name_en}
            </Link>
          </h3>
          <span className="flex items-center gap-2 font-medium text-lg text-[#0C2756]">
            {item.offer_price && item.offer_price > 0 ? (
              <>
                <span>{`${locale === "ar" ? "ج.م" : "$"} ${item.offer_price}`}</span>
                <span className="line-through text-[#0C2756]/60">{`${locale === "ar" ? "ج.م" : "$"} ${item.price}`}</span>
              </>
            ) : (
              <span>{`${locale === "ar" ? "ج.م" : "$"} ${item.price}`}</span>
            )}
          </span>
        </div>
      </div>
