...
const CartSidebarModal = () => {
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-99999 overflow-y-auto no-scrollbar w-full h-screen bg-[#0C2756]/70 ease-linear duration-300 ${
        isCartModalOpen ? "translate-x-0" : "translate-x-full"
      }`}
    >
      <div className="flex items-center justify-end">
        <div className="w-full max-w-[500px] shadow-1 bg-white  px-4 sm:px-7.5 lg:px-11 relative modal-content">
          <div className="sticky top-0 bg-white flex items-center justify-between pb-7 pt-4 sm:pt-7.5 lg:pt-11 border-b border-[#239FBF] mb-7.5">
            <h2 className="font-medium text-[#0C2756] text-lg sm:text-2xl">
              {t("cartView")}
            </h2>
            <button
              onClick={() => closeCartModal()}
              aria-label="button for close modal"
              className="flex items-center justify-center ease-in duration-150 text-[#0C2756] hover:text-[#239FBF]"
            >
              <svg
          ...
          <div className="border-t border-[#239FBF] bg-white/20 pt-5 pb-4 sm:pb-7.5 lg:pb-11 mt-7.5 sticky bottom-0">
            <div className="flex items-center justify-between gap-5 mb-6">
              <p className="font-medium text-xl text-[#0C2756]">{t("subtotal")}</p>
              <p className="font-medium text-xl text-[#0C2756]">${totalPrice}</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <Link
                onClick={() => closeCartModal()}
                href={`/${locale}/cart`}
                className="w-full flex justify-center font-medium text-white bg-[#239FBF] py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[#0C2756]"
              >
                {t("viewCart")}
              </Link>
              ...
              <Link
                ...
                className="w-full flex justify-center font-medium text-white bg-[#0C2756] py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-opacity-95"
              >
                {t("checkout")}
              </Link>
            </div>
